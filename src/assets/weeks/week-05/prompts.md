...
| (폐기) paired-bootstrap.png 생성본 | 쌍체 부트스트랩 | 1차본 원래 채점셋 8장 대 가상 채점셋 7장(크기 불일치). 2차본 카드마다 A/B 중 하나만 표시(쌍체 아님). 3차본은 카드 6장(4/6 대 3/6, δ = 0.17)에 δ = 0.04 라벨이라 모순(검토에서 지적). 4차본은 프롬프트의 설명 문장이 그림 속 영어 글자로 찍힘. 5차본은 캡션을 공식으로 바꿨으나 카드가 X 3/6 대 Y 3/6(δ = 0)인데 분포가 양수 δ 중심이라 OCR 리뷰에서 Important로 지적. 생성 이미지로는 카드 조합을 보장할 수 없어 matplotlib 그림으로 교체(아래 표). 2차 생성 중 1회 job failed(원인 미표시) 후 재시도 성공 |
...
생성 이미지는 수치 위치가 정확하지 않을 수 있으므로, 실측값이나 정확한 조합을 보여야 하는 그래프 3장은
...
| paired-bootstrap.png | 쌍체 부트스트랩, 6문항 X 4/6 대 Y 3/6 | 카드 조합을 코드로 고정하고 실제로 복원추출(시드 42). 가상 채점셋 3개 δ -0.17, +0.33, +0.17. 1만 번 중 δ <= 0 비율 38%, SLP3 p = 0.40 |
